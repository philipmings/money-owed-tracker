...
const patch = fs.readFileSync('ui-v20-patch.js','utf8');
const patchCss = fs.readFileSync('ui-v20-patch.css','utf8');
...
assert.ok(patch.includes('syncFieldState'));
assert.ok(patch.includes('node.disabled=adjust'));
assert.ok(patch.includes('node.disabled=!adjust'));
assert.ok(patch.includes('loadFlowActivity'));
assert.ok(patch.includes('Flow.groupFlowTransactions'));
assert.ok(css.includes('.v20-account-card'));
assert.ok(css.includes('.v20-flow-review'));
assert.ok(patchCss.includes('.v20-flow-group'));
...
assert.ok(shell.includes('ui-v20-patch.js?v=20'));
assert.ok(shell.includes('ui-v20-patch.css?v=20'));
...
assert.ok(sw.includes("'./ui-v20-patch.js?v=20'"));
